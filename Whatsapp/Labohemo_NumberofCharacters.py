...
for person in people:
    messages[person]=sum(len(i) for i in instancedict[person].messages)
...
messages["Emre Çarkcı"]=messages["Emre Çarkcı"]+messages["Emre"]
del messages["Emre"]
...
my_norm =SymLogNorm(linthresh=0.03, linscale=0.03, base=2,vmin=5, vmax=175)
fig,ax=plt.subplots()

numberofmessages=np.asfarray(numberofmessages)/1000
plt.ylabel("Karakter Sayısı (x1000)",size=15)
plt.title("Toplam Karakter: 1.089 Milyon")
...
plt.tight_layout()
plt.savefig(r"C:\Users\Egecan\Desktop\Yazılan Toplam Karakter.png",dpi=600)